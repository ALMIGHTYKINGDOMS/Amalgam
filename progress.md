...
- [x] After the previously running launcher exited naturally, bound the clean final EXE/DLL into the canonical `cpp/build-release` output. Canonical hashes now match the fresh candidate (`5207711570D3AC71107DE04D4C8834CEE0245C4CCEED1589E7980C0E11EFD60D` / `787B3F9E58BD4C60308801F380DE69DCF05EBFA68FE54F03045CBD19DB35497B`) and canonical CLI smoke passed.
...
- [x] Canonical executable lock cleared when the active process exited; `cpp/build-release/amalgam_launcher.exe` is now bound to the clean final hash and its preflight/smoke checks pass.

## 2026-09-24 — Final deep audit and responsive polish

- [x] Closed the last repository-resolvable visual defect found in the final audit: Microsoft approval-pending and error dialog states now size to their content instead of reserving the full sign-in wizard height. The fixture and live dialog paths share the same compact sizing policy, while the device-code state keeps the larger layout it needs.
- [x] Rebuilt the current Release sources. The canonical link was still correctly blocked by the already-running `amalgam_launcher.exe` (PID 24232); the separately named verification binary `cpp/build-release/amalgam_launcher_bedrock_auth.exe` linked successfully with SHA-256 `B45EB473318A0ADDF2B59FA6088E66BB301949D84188C2612750458388D8B19B`.
- [x] Fresh native regression passed 45/45. Launcher preflights `--check-prereqs`, `--check-java`, and `--check-official-launcher` all exited 0 against that exact verification binary.
- [x] Supporting gates passed again: runtime-agent 38/38, Bedrock static truth 4/4 plus validator, update-manifest 12/12, secret-scanner tests 2/2, scoped source secret scan across 287 text files with 0 secret-category matches, Supabase source verification (26 migrations / 24 Edge Functions), and production-schema verification 66/66.
- [x] Refreshed the affected responsive visual evidence at both host-valid tiers. The immutable run `artifacts/audit-2026-09-22/launcher/final-deep-audit-bedrock-auth-polished-20260924T040755073Z-a33f709b` contains 8/8 passed captures (Bedrock Coming Soon, Bedrock responsive states, and Microsoft approval-pending fallback at 1256x624 and 960x600). Manual inspection confirms no nested/horizontal scroll on Bedrock and no dead-space/clipping defect in the compact approval dialog.
- [x] Completed a read-only live Supabase audit for project `nnrrmvaxnoknthpwvttt`: project URL, database/schema reachability, migration/function inventory, and representative schema/auth/protected-function checks are consistent with the local release contract. Live advisor output contains intentional/non-blocking WARN/INFO items (user-facing SECURITY DEFINER RPC policy review, RLS init-plan/index tuning, and the dashboard-only leaked-password-protection setting); no P0/P1 launcher defect was found and no risky live DDL was applied during this pass.
- [x] No Bedrock runtime was launched or detected, and the active Minecraft/official launcher session was preserved. Fixture screenshots remain explicitly inert and are not evidence of a live Bedrock installation.
- [ ] Owner-controlled gates remain: obtain Microsoft/Xbox production approval, apply trusted Windows signing/timestamping, publish and verify the signed update feed, complete authenticated live CurseForge/provider verification, enable Supabase leaked-password protection, and perform clean-machine/real Java handoff certification. These are not silently claimed as complete by this audit.

## 2026-09-24 — Repository truth alignment and fresh immutable package

- [x] Removed beta-era installer version comments/defaults from `.github/workflows/build.yml`; CI now defaults to the official `1.0.0` installer version.
- [x] Aligned README, User Guide, Release Notes, static website copy, and operational-readiness reporting with the actual 1.0.0 product boundary: Bedrock is Coming Soon with static package validation only; no Bedrock runtime activity is claimed.
- [x] Rebuilt the clean native candidate tree `cpp/build-release-final`; CTest passed 45/45 again. Launcher and DLL hashes remain `5207711570D3AC71107DE04D4C8834CEE0245C4CCEED1589E7980C0E11EFD60D` and `787B3F9E58BD4C60308801F380DE69DCF05EBFA68FE54F03045CBD19DB35497B`.
- [x] Created the fresh candidate at `artifacts/release-candidate-2026-09-24/rc-20260924T044334Z`. ZIP SHA-256 is `18AF3E98E0A456C833DF845D5FC91E67F4EB5A362B2622890514432F026AF911`; size is 113,006,167 bytes.
- [x] Exact ZIP validation, clean extracted-runtime validation, installer-input QA, launcher smoke checks, component-manifest binding, and candidate secret checks passed. The local update feed was regenerated and verified with manifest SHA-256 `DE58572EB7CCF2394951822C9F28961EB2D4D596E56456D83F467837E8BC7075`.
- [x] The previously validated installer remains available at `artifacts/release-candidate-2026-09-24/rc-20260924T043125Z/installer/AmalgamLauncher-1.0.0-Setup.exe` and its sandbox install/uninstall evidence remains valid. The local Inno compiler is currently unavailable without elevation, so no stale installer was relabelled as the exact new candidate; the corrected CI workflow will produce the matching installer when run with the compiler.
- [ ] External owner gates remain: trusted Authenticode certificate, Microsoft/Xbox approval, publication/verification of the signed feed and website release, Supabase leaked-password protection, clean-machine Java handoff, and a Windows build host. GitHub Actions currently rejects the workflow before startup because the repository account has a billing lock; no hosted installer build is claimed. Bedrock remains runtime-excluded.
