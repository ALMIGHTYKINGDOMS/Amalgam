...
- [ ] Owner-controlled gates remain: close the active launcher before replacing/binding the canonical executable, obtain Microsoft/Xbox production approval, apply trusted Windows signing/timestamping, publish and verify the signed update feed, complete authenticated live CurseForge/provider verification, enable Supabase leaked-password protection, and perform clean-machine/real Java handoff certification. These are not silently claimed as complete by this audit.
...
- [ ] External owner gates are unchanged: trusted Authenticode certificate, Microsoft/Xbox approval, publication/verification of the signed feed and website release, Supabase leaked-password protection, clean-machine Java handoff, and the active-launcher close/rebind step. Bedrock remains runtime-excluded.
