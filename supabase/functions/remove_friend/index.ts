...
Deno.serve(async (request) => {
  ...
  const { data: limited } = await supabase.rpc("enforce_rate_limit", { p_action: "remove_friend", p_max_per_minute: 20 });
  if (limited === false) return jsonResponse({ error: "rate limit exceeded" }, 429, request);
  try {
    const body = await readJson(request);
    const validation = validateUuid(body.friend_id, "friend_id");
    if (validation) return jsonResponse({ error: validation }, 400, request);
    const { data, error: rpcError } = await supabase.rpc("remove_friend", { p_friend_id: body.friend_id });
