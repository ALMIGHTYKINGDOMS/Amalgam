...
Deno.serve(async (request) => {
  ...
  const { data: limited } = await supabase.rpc("enforce_rate_limit", { p_action: "leave_party", p_max_per_minute: 20 });
  ...
  try {
    ...
    const { data, error: rpcError } = await supabase.rpc("leave_party", { p_party_id: body.party_id });
