...
Deno.serve(async (request) => {
  ...
  const { data: limited } = await supabase.rpc("enforce_rate_limit", { p_action: "accept_friend_request", p_max_per_minute: 20 });
  if (limited === false) return jsonResponse({ error: "rate limit exceeded" }, 429, request);
  try {
    const body = await readJson(request);
    const requestId = body.request_id;
    const validation = validateUuid(requestId, "request_id");
    if (validation) return jsonResponse({ error: validation }, 400, request);
    const { data, error: rpcError } = await supabase.rpc("accept_friend_request", { p_request_id: requestId });
