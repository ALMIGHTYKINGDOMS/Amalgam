...
Deno.serve(async (request) => {
  ...
  const { data: limited } = await supabase.rpc("enforce_rate_limit", { p_action: "reject_friend_request", p_max_per_minute: 20 });
  if (limited === false) return jsonResponse({ error: "rate limit exceeded" }, 429, request);
  try {
    const body = await readJson(request);
    const validation = validateUuid(body.request_id, "request_id");
    if (validation) return jsonResponse({ error: validation }, 400, request);
    const { data, error: rpcError } = await supabase.rpc("reject_friend_request", { p_request_id: body.request_id });
